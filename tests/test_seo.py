...
def test_construye_indice_sitemap_de_generos_y_de_peliculas(tmp_path):
    manifiesto = seo.construir(
        tmp_path,
        "https://nort5.com/",
        [entrada("the_godfather"), entrada("alien", "horror")],
        [{"genre": "drama"}, {"genre": "horror"}],
    )

    assert manifiesto["site_url"] == "https://nort5.com"
    assert manifiesto["sitemaps"] == ["sitemap-genres.xml", "sitemap-movies-0001.xml"]

    peliculas = (tmp_path / "seo" / "sitemap-movies-0001.xml").read_text()
    assert "https://nort5.com/movie/the_godfather" in peliculas
    assert "https://nort5.com/movie/alien" in peliculas
    ...
    generos = (tmp_path / "seo" / "sitemap-genres.xml").read_text()
    assert "https://nort5.com/genre/horror" in generos
...
def test_escapa_lo_que_hay_que_escapar(tmp_path):
    ...
    xml = (tmp_path / "seo" / "sitemap-movies-0001.xml").read_text()
...
def test_no_reescribe_un_sitemap_identico(tmp_path):
    ...
    fichero = tmp_path / "seo" / "sitemap-movies-0001.xml"
